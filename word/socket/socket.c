...
uint8 socket(SOCKET s, uint8 protocol, uint16 port, uint8 flag)
{
   ...
   {
      close(s);
		 //printf("close socket 2，写入相应控制模式");
...
uint8 connect(SOCKET s, uint8 * addr, uint16 port)
{
    ...
    else							// 如果目的IP和Port未设置，则进行设置
    {
        ...
        IINCHIP_WRITE( Sn_DPORT1(s), (uint8)(port & 0x00ff));
				...
				IINCHIP_WRITE( Sn_CR(s) ,Sn_CR_CONNECT);						// Sn_CR数值设为0x04，并执行TCP连接请求命令

        while ( IINCHIP_READ(Sn_CR(s) ) ) ;									// MCU读取Sn_CR(s)的数值

        while ( IINCHIP_READ(Sn_SR(s)) != SOCK_SYNSENT )		// 此时Sn_SR(s)寄存器应该处于SOCK_SYNSENT，下面排除不在该状态的几种情况
        {
					
            if(IINCHIP_READ(Sn_SR(s)) == SOCK_ESTABLISHED)	// Socket连接已经建立，正常连接
            {
            ...
            {
                IINCHIP_WRITE(Sn_IR(s), (Sn_IR_TIMEOUT));   // 通知MCU该中断，并清中断
                ret = 0;																		// ret置0，连接中断
                break;
            }
        }
