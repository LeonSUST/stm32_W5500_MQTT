...
extern void setkeepalive(SOCKET s);
...
/*"0x00XXXX"即"0000 0000 XXXX XXXX XXXX XXXX"。高16位为地址段（偏移地址），定义了控制段所指定寄存器的偏移地址；
低8位为控制段（其中前5位为区域选择位BSB，中1位为R/W，后2位为OM位），指定要对哪个寄存器操作。
"ch"定义为Socket寄存器的编号，从0~7。
		BSB：	00000 通用寄存器   			
					00001	Socket0寄存器			ch=0
					00101	Socket1寄存器			ch=1
					01001	Socket2寄存器			ch=2
					01101 Socket3寄存器			ch=3
					10001 Socket4寄存器			ch=4
					10101 Socket5寄存器			ch=5
					11001 Socket6寄存器			ch=6
					11101 Socket7寄存器			ch=7		*/
