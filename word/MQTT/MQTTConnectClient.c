...
int MQTTSerialize_connect(unsigned char* buf, int buflen, MQTTPacket_connectData* options)
{
	...
	MQTTHeader header = {0};//申请一个MQTTHeader类型的联合体变量并初始化，该联合体变量存放的是固定头部内容
	MQTTConnectFlags flags = {0};//申请一个MQTTConnectFlags类型的联合体变量并初始化，该联合体变量存放的是连接标志内容
	int len = 0;
	int rc = -1;
...
//填写固定头部
  ...
  //Connect flags 字段
	flags.all = 0;
	flags.bits.cleansession = options->cleansession;//Clean session flag 为0时服务器存储客户端的订阅信息，为1时丢弃任何之前维护的信息
